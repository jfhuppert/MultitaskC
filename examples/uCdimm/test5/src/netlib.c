...
/*----------------------------------------------------------------------------*/
void networkp_init(struct ip_addr *ia,struct ip_addr *nm,struct ip_addr *gw,struct ip_addr *dip)
{
unsigned long
	_a,_b,_c,_d;
...
char
	*var;
	...
	var=getevar("DI");
	if(var==0) var="192.168.26.200";
	ip_convert(var,&_a,&_b,&_c,&_d);
	IP4_ADDR(dip,_a,_b,_c,_d);
	putstr("Using Dest IP address    : "); putstr(var); putstr("\r\n");
}
/*----------------------------------------------------------------------------*/
